...
def draw_gene_v1(gene_info, id, d, scale, leftmost, topmost, ort='+', bg='NA', ed='NA', arrow=True, name=True):
	...
	if arrow:
		...
		if name:
			if strand == '+':
				pcRotate = 'rotate(315, %s, %s)' % (path_x, path_y-8)
				d.add(d.text(id, insert = (path_x, path_y-8), font_size = 5, fill = 'black', font_family = 'Arial', transform = pcRotate))
			else:
				pcRotate = 'rotate(315, %s, %s)' % (path_x+h+l_x, path_y-8)
				d.add(d.text(id, insert = (path_x+h+l_x, path_y-8), font_size = 5, fill = 'black', font_family = 'Arial', transform = pcRotate))
	else:
		if ort == '+':
			x = start-bg+1
		elif ort == '-':
		...
		if name:
			pcRotate = 'rotate(315, %s, %s)' % (x, yloc-5-8)
			d.add(d.text(id, insert = (x, yloc-5-8), font_size = 5, fill = 'black', font_family = 'Arial', transform= pcRotate))
	return d
...
def draw_gene_model(cds_list, id, d, scale, leftmost, topmost, ort='+', bg='NA', ed='NA', arrow=True, name=True):
	...
	for info in cds_list:
		...
		tri_y1, tri_y2, tri_y3 = yloc, yloc+5, yloc-5
